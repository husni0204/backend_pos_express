    ...
    try {
        const data = await prisma.$transaction(async (prisma) => {
            ...
            for (let i = 0; i < value.detail.length; i++) {
                ...
                if (Number(value.detail[i].qty) == 0 || value.detail[i].qty == "" || value.detail[i].product == null) {
        ...
        return res.status(200).json({
            message: "success insert order return",
            result: data,
        });
    } catch (error) {
        logger.error("controllers/orderReturn.controller.js:insertOrderReturn - " + error.message);
