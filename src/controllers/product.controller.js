...
import { setCode } from "../utils/doccumentPattern.js";
...
export const createProduct = (req, res) => {
    ...
    const allwFileExt = process.env.FILE_EXTENSION;
...
export const getAllProducts = async (req, res) => {
    ...
    try {
        ...
        logger.error("controllers/product.controller.js:getAllProducts -> " + error.message);
        res.status(500).json({
            message: error.message,
            result: null,
            lastId: result.length > 0 ? result[result.length - 1].id : 0,
...
export const updateProduct = async (req, res) => {
    ...
    if (!req.files || req.files === null || req.files.file === undefined || !req.files.file) {
        ...
        if (!allowedType.includes(ext.toLowerCase())) return res.status(422).json({ message: "Invalid image type", result: null });
        if (fileSize > fileMaxSize)
            return res.status(422).json({
                message: msgFileSize,
                result: null,
