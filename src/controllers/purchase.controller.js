...
import { setOrderCode } from "../utils/doccumentPattern.js";
    ...
    try {
        ...
        await prisma.$transaction(async (prisma) => {
            ...
            for (let i = 0; i < value.detail.length; i++) {
                ...
                if (value.detail[i].product.productId == "" || value.detail[i].product.productId == null) {
        ...
        logger.error("controllers/purchase.controller.js:createPurchase - " + error.message);
...
export const getAllPurchase = async (req, res) => {
    ...
    try {
      ...
      `;
        ...
        return res.status(200).json({
            message: "success get all purchase",
            result: result,
            lastId: result.length > 0 ? result[result.length - 1].id : 0,
            hasMore: result.length >= limit ? true : false,
        });
    } catch (error) {
        logger.error("controllers/purchase.controller.js:getAllPurchase - " + error.message);
        return res.status(500).json({
            message: error.message,
            result: null,
            lastId: result.length > 0 ? result[result.length - 1].id : 0,
...
export const getPurchaseById = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({
            message: "success get purchase by id",
            result: result,
        });
    } catch (error) {
        logger.error("controllers/purchase.controller.js:getPurchaseById - " + error.message);
        return res.status(500).json({
            message: error.message,
            result: null,
        });
...
export const generatePdf = async (req, res) => {
    ...
    try {
        ...
        if (process) {
            return res.status(200).json({
                message: "success generate pdf purchase",
                result: "/pdf/" + fileName,
            });
        }
    } catch (error) {
        logger.error("controllers/purchase.controller.js:generatePdf - " + error.message);
        return res.status(500).json({
            message: error.message,
            result: null,
        });
...
export const generateExcel = async (req, res) => {
    ...
    WHERE o.date BETWEEN ${startDate} AND ${endDate}`;
        ...
        return res.status(200).json({
            message: "success generate excel purchase",
            result: `/excel/purchase.xlsx`,
        });
    } catch (error) {
        logger.error("controllers/purchase.controller.js:generateExcel - " + error.message);
        return res.status(500).json({
            message: error.message,
            result: null,
        });
...
export const purchaseYearly = async (req, res) => {
    ...
    WHERE YEAR(date) = ${year}`;
        ...
        return res.status(200).json({
            message: "success get purchase yearly",
            result: arry,
        });
    } catch (error) {
        logger.error("controllers/purchase.controller.js:purchaseYearly - " + error.message);
        return res.status(500).json({
            message: error.message,
            result: null,
        });
